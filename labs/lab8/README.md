...
| ---------------- | --------------------------- | ----------------------- | ---------------------- |
| `couchbase_lab8` | `couchbase:community-7.2.4` | 8091 (UI), 8093 (Query) | `admin` / `Admin!1234` |

Two init containers run automatically:

- `couchbase_init_lab8` — waits for the Couchbase REST API, then creates the
  `northwind` bucket, sets memory quotas, and creates a local `student` user
- `couchbase_import_lab8` — waits for the cluster to be ready, then imports
  all Northwind JSON files via `cbimport`, including the pre-nested
  `orders_nested` collection

Lab 8 uses the `init` Makefile profile, so `make up LAB=lab8` automatically
includes both init containers.

## Start / stop

```bash
make up LAB=lab8       # starts Couchbase + runs both init containers
make down LAB=lab8
make clean LAB=lab8
```
...
```bash
make pdf LAB=lab8
make pdf LAB=lab8 TARGET=solution
```
...
├── scripts/
│   ├── init.sh     # run by couchbase_init_lab8: creates bucket, memory settings,
│   │               # indexes, and the student user via the REST API
│   └── import.sh   # run by couchbase_import_lab8: calls cbimport for each

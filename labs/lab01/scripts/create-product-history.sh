...
bash ${ROOT}/common/scripts/postgres-transaction.sh postgres_server_lab01 "$POSTGRES_SQL_FILE"
echo "Product History setup in PostgreSQL completed successfully."

echo "Setting up Product History in SQL Server..."
bash ${ROOT}/common/scripts/mssql-transaction.sh mssql_server_lab01 "$MSSQL_SQL_FILE"
echo "Product History setup in SQL Server completed successfully."

echo "Setting up Product History in SQLite..."
bash ${ROOT}/common/scripts/sqlite-transaction.sh sqlite_server_lab01 db/northwind-lab01.db "$SQLITE_SQL_FILE"

...
| --------- | ----- | ---- | ----------- |
| `mssql_server_lab01` | `mssql/server:2022-latest` | 1433 | `sa` / `Admin!1234` |
| `postgres_server_lab01` | `postgres:16` | 5432 | `admin` / `admin`, db `postgres` |
| `sqlite_server_lab01` | `alpine` | — | file-based |

## Start / stop

```bash
make up LAB=lab01      # start all three containers
make down LAB=lab01    # stop, keep volumes
make clean LAB=lab01   # stop, remove volumes and generated db/ files
```

## Render report

```bash
make pdf LAB=lab01              # render template/report.md → template/report.pdf
make pdf LAB=lab01 TARGET=solution  # render solution
```

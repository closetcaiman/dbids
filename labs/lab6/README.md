...
| ----------------- | ------------------------- | --------------------------- | -------------------------------- |
| `postgres_lab6`   | `postgres:16`             | 5432                        | `admin` / `admin`, db `postgres` |
| `clickhouse_lab6` | `clickhouse-server:25.12` | 8123 (HTTP), 19000 (native) | `admin` / `admin`, db `ds_lab`   |
...
```bash
make pdf LAB=lab6
make pdf LAB=lab6 TARGET=solution
```
